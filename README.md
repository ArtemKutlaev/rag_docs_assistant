...
Веб-приложение, которое превращает учебные материалы в персонального ассистента: студент загружает PDF (учебник, конспект, слайды лекций), система индексирует текст в векторной базе и отвечает на вопросы по содержимому с помощью **YandexGPT**. Есть регистрация, JWT-авторизация, публичная и личная библиотеки, чтение и скачивание файлов.

![Чат по книге: вопрос о персептроне и ответ ассистента](docs/screenshots/06-chat.png)
...
Публичные книги можно шарить с однокурсниками; личные материалы (свои лекции, черновики) остаются только у владельца.
...
`ACCESS_TOKEN_EXPIRE_MINUTES` должен быть целым числом: при старте приложения значение читается через `int(...)`.
